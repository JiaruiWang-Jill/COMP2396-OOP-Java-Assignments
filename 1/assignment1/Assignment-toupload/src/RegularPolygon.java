...
public class RegularPolygon extends Shape{
	...
	*or on any of the sides or vertices of the polygon
	...
	{	
		...
		for(int i=0;i<num;i++) 
			{	
				if(this.getXLocal()[num/2] -temp_x> 0.01 ) 
				{
						return false;
				} 
				...
				double h = temp_x*Math.sin(i*2*Math.PI/getNumOfSides())+temp_y*Math.cos(i*2*Math.PI/getNumOfSides());
				
				if(this.getXLocal()[num/2] - s > 0.01 ) 
				{
				return false;
				} 
			}
